...
pub struct DataDictionary {
    ...
    fields_by_tag: HashMap<u32, FieldEntry>, // tag to its details mapping
    tag_name_map: HashMap<String, u32>,      // name of tag to its number
    components: HashMap<String, ComponentEntry>, // name of component to component details
    groups: HashMap<String, Group>,          // name of group to group details
    messages: HashMap<String, MessageEntry>, // message type to message details map
}
...
impl DataDictionary {
    ...
    pub fn add_message(&mut self, m_type: &str, m_entry: MessageEntry) {
        self.messages.insert(m_type.to_string(), m_entry);
    }

    fn get_message(&self, m_type: &str) -> Option<&MessageEntry> {
        // get message by message type
        self.messages.get(m_type)
    }

    fn get_message_mut(&mut self, m_type: &str) -> Option<&mut MessageEntry> {
        self.messages.get_mut(m_type)
    }
    ...
    pub fn is_valid_tag_value(&self, msg_type: &str, tag: u32, val: &str) -> bool {
        // Returns true or false based on if the tag is valid according to message
        // and the value is valid according to tag
        let field_entry = match self.get_field(&tag) {
            Some(m) => m,
            None => return false, // tag does not exist in data dictionary
        };
        // tag exist with the tag
        let msg_entry = match self.get_message(msg_type) {
            Some(m) => m,
            None => return false, // message entry not found. invalid message type
        };
        let mut tag_found = false;
        let tag_name = field_entry.name();
        // check if tag name is present in message fields
        if msg_entry.msg_fields.contains_key(tag_name) {
            // found in message fields
            tag_found = true;
        } else {
            // check if tag name is part of message component
            for (comp_name, _) in msg_entry.msg_component.iter() {
                let comp = self.get_component(comp_name);
            }
        }
...
pub struct FieldEntry {
    // tag number, name, its type & what values it can take
    number: u32,
    name: String,
    ftype: String,
...
impl FieldEntry {
    ...
    fn is_valid_value(&self, val: &str) -> bool {
        for field_entry in self.values() {
            if field_entry.value.eq(val) {
                return true;
            }
        }
        return false;
...
pub struct ComponentEntry {
    comp_name: String,
    comp_groups: HashMap<String, bool>, // group name & required or not
    comp_fields: HashMap<String, bool>, // field name & required or not
}
            ...
            match msg_child_node.tag_name().name() {
                ...
                "group" => {
                    ...
                    if dict.get_group(grp_name).is_none() {
                        let mut group = Group::new(grp_name);
                        for group_field in msg_child_node.children().filter(|n| {
                            n.node_type() == NodeType::Element && n.has_tag_name("field")
                        }) {
                            group.add_field(
                                group_field.attribute("name").unwrap(),
                                group_field.attribute("required").unwrap(),
                ...
                "component" => {
                    ...
                    // TODO: actually component can be flattened in the message
                }
                _ => {} // TODO: Error handling
            }
        }
        dict.add_message(msg_type, message);
    }
}
